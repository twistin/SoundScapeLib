...
const LibraryPlayer: React.FC<LibraryPlayerProps> = ({ file }) => {
  ...
  useEffect(() => {
    if (containerRef.current && !wavesurferRef.current) {
      ...
      wavesurferRef.current.on('error', (e: any) => {
          ...
          // If not explicitly handled by loadMediaElement's onerror, catch here
          setLoadError("Failed to load audio. Check format or network.");
      });
  ...
  useEffect(() => {
    if (file && wavesurferRef.current) {
      ...
      // CORS Handling:
      // Try anonymous first to enable WebAudio Analysis.
      // If error occurs (likely CORS block or format), retry without crossOrigin.
      ...
      media.onerror = () => {
          ...
          console.warn("Library Player Audio Error:", errCode, media.error?.message);
          
          if (media.crossOrigin === 'anonymous') {
              console.log("Retrying playback without CORS headers...");
              media.crossOrigin = null; // Remove attribute
              media.src = file.url; // Force reload
              wavesurferRef.current.loadMediaElement(media); // Reload into wavesurfer
          } else {
              setLoadError("Audio format not supported or access denied.");
          }
      };
  ...
  const setupAudioGraph = () => {
      try {
        ...
        // If CORS is blocked, we can't create source.
        if (!sourceRef.current) {
             try {
                sourceRef.current = ctx.createMediaElementSource(media);
             } catch(e) {
                 console.warn("Could not create MediaElementSource. Visualizer disabled due to CORS.", e);
                 return;
             }
        }
        ...
        startAnalysisLoop();
          ...
          console.warn("Audio Graph Setup Error", e);
      }
  };
  ...
  // ... [Buffer Manipulation functions are standard and assumed correct] ...
      ...
      <div className="p-4 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        <div className="flex items-center gap-4 w-full lg:w-auto">
           ...
           <div className="min-w-0">
             ...
             {loadError && <p className="text-xs text-red-400 font-bold mt-1">{loadError}</p>}
           </div>
        </div>
