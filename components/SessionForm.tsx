...
const SessionForm: React.FC<SessionFormProps> = ({ onSubmit, onCancel, initialData, projects, defaultProject }) => {
  ...
  const FileInput = ({ label, file, onChange, accept }: { label: string; file: File | null; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; accept: string }) => {
    const inputId = `file-${label.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}`;
    return (
      <div className="w-full">
          <label htmlFor={inputId} className="block text-sm font-medium text-slate-400 mb-2">{label}</label>
          <div className="mt-2 flex justify-center rounded-lg border border-dashed border-slate-600 px-6 py-10">
              <div className="text-center">
                  ...
                  <div className="mt-4 flex text-sm leading-6 text-slate-400">
                      <label htmlFor={inputId} className="relative cursor-pointer rounded-md font-semibold text-cyan-400 focus-within:outline-none focus-within:ring-2 focus-within:ring-cyan-500 focus-within:ring-offset-2 focus-within:ring-offset-slate-900 hover:text-cyan-300">
                          <span>Upload a file</span>
                          <input id={inputId} name={inputId} type="file" className="sr-only" onChange={onChange} accept={accept} />
                      </label>
